...
function Comments(props) {
  ...
  const sendComment = async (e) => {
    ...
    if (ısAuthenticated) {
      ...
      try {
        ...
        const commentVariable = await axios.post("/api/comment", variable, {
  ...
  return (
    <div>
      <div className="postBody_icons">
        <div className="postBody_container">
          ...
          >
            ...
            <Button sx={{ p: 2, fontSize: 12 }} style={{ color: "black" }}>
              Newest first
            </Button>
          </Popover>
      ...
      {!props.CommentLists.length ? <Empty description="No Comment" /> : <></>}
    </div>
  );
}
