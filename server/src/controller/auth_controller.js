...
const verifymail = async (req,res,next)=>{
    ...
    if(token){
        ...
        try{
          const verifyI =  await jwt.verify(token,process.env.CONFIRM_MAIL_JWT_SECRET)
          ...
          if(!verifyI){
           ...
           const userTrueControl = await User.findOne({"local.email":verifyI.mail})
         
            
          if(!userTrueControl.local.emailactive){
            const sonuc = await User.findByIdAndUpdate(tokenıd,{"local.emailactive":true})
 ...
 const createActivationToken =  (payload)=>{
   ...
   const jwtoken =  jwt.sign(jwtınfos,process.env.CONFIRM_MAIL_JWT_SECRET,{expiresIn:"2m"})
...
return jwtoken
 ...
 const generateAccessToken = (payload) => {
    ...
    return jwt.sign(jwtınfos, process.env.RESET_PASSWORD_JWT_SECRET, {expiresIn: '2m'})
  }
...
const register = async (req,res,next)=>{
    ...
    if(_user && _user.local.emailactive == true){//eger verıtabanında varsa o kısı bura calısır 
...
const forgetPass = async (req,res,next)=>{
    ...
    try{
        ...
        if(_user && _user.local.emailactive == true){
            ...
            const url = process.env.WEB_SITE_URL+'auth/resetPassword/'+activatTokenForPass 
...
const resetPassword = async (req,res,next)=>{
    ...
    if(token){
        try{
            ...
            const ıdUser =  await jwt.verify(token,process.env.RESET_PASSWORD_JWT_SECRET)
            ...
            const findUser = await User.findOne({"local.id":ıdUser.id}) //db passwordfindfor
               ...
               if(Number(timeValues) > Number(tokenTime) ){
                ...
                if(passCompare){
                    ...
                    await User.findByIdAndUpdate(ıdUser.id,{"local.password":passUpdate,"local.passwordResetTime":now})
